...
def create_dataloader(path, imgsz, batch_size, stride, single_cls, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
                      rank=-1, world_size=1, workers=1, image_weights=False, quad=False, prefix='', tidl_load=False, kpt_label=False):
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    def __init__(self, path, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        try:
            ...
            sorted_index = [i[0] for i in sorted(enumerate(self.img_files), key=lambda x:x[1].split('_')[-1])]
            ...
            try:
                ...
                if os.path.isfile(lb_file):
                    ...
                    if len(l):
                        ...
                        if kpt_label:
                            assert l.shape[1] == 21, 'labels require 56 columns each' # all key points number: 56.
                            assert (l[:, 6::3] <= 1).all(), 'non-normalized or out of bounds coordinate labels'
                            assert (l[:, 7::3] <= 1).all(), 'non-normalized or out of bounds coordinate labels'
                            # print("l shape", l.shape)
                            kpts = np.zeros((l.shape[0], 16))
                            for i in range(len(l)):
                                kpt = np.delete(l[i,6:], np.arange(2, l.shape[1]-6, 3))  #remove the occlusion paramater from the GT
                                kpts[i] = np.hstack((l[i, :6], kpt))
                            l = kpts
                            assert l.shape[1] == 16, 'labels require 39 columns each after removing occlusion paramater'
                        else:
                            assert l.shape[1] == 6, 'labels require 5 columns each'
                            assert (l[:, 1:6] <= 1).all(), 'non-normalized or out of bounds coordinate labels'

                        assert np.unique(l, axis=0).shape[0] == l.shape[0], 'duplicate labels'
                    else:
                        ne += 1  # label empty
                        l = np.zeros((0, 16), dtype=np.float32) if kpt_label else np.zeros((0, 6), dtype=np.float32)
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            if labels.size:  # normalized xywh to pixel xyxy format
                labels[:, 2:] = xywhn2xyxy(labels[:, 2:], ratio[0] * w, ratio[1] * h, padw=pad[0], padh=pad[1], kpt_label=self.kpt_label)
        ...
        if nL:
            ...
            if self.kpt_label:
                labels[:, 7::2] /= img.shape[0]  # normalized kpt heights 0-1
                labels[:, 6::2] /= img.shape[1] # normalized kpt width 0-1
        ...
        if self.augment:
            ...
            if random.random() < hyp['flipud']:
                ...
                if nL:
                    labels[:, 3] = 1 - labels[:, 3]
                    if self.kpt_label:
                        labels[:, 7::2]= (1-labels[:, 7::2])*(labels[:, 7::2]!=0)
            ...
            if random.random() < hyp['fliplr']:
                ...
                if nL:
                    ...
                    if self.kpt_label:
                        ...
                        labels[:, 7::2] = labels[:, 7::2][:, self.flip_index]

        num_kpts = (labels.shape[1]-6)//2
        labels_out = torch.zeros((nL, 7+2*num_kpts)) if self.kpt_label else torch.zeros((nL, 7))
        if nL:
            if  self.kpt_label:
                labels_out[:, 1:] = torch.from_numpy(labels)
            else:
                labels_out[:, 1:] = torch.from_numpy(labels[:, :6])
    ...
    if n:
        ...
        if use_segments:  # warp segments
            ...
            if kpt_label:
                ...
                xy_kpts[:, :2] = targets[:,6:].reshape(n * 5, 2)  #num_kpt is hardcoded to 17
        ...
        # filter candidates
        i = box_candidates(box1=targets[:, 2:6].T * s, box2=new.T, area_thr=0.01 if use_segments else 0.10)
        targets = targets[i]
        targets[:, 2:6] = new[i]
        if kpt_label:
            targets[:, 6:] = xy_kpts[i]
