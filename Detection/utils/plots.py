...
def plot_one_box(x, im, color=None, label=None, line_thickness=3, kpt_label=False, kpts=None, steps=2, orig_shape=None, kpt_color=None):
    ...
    if kpt_label:
        plot_skeleton_kpts(im, kpts, steps, orig_shape=orig_shape, kpt_color=kpt_color)


def plot_skeleton_kpts(im, kpts, steps, orig_shape=None, kpt_color=None):
    ...
    palette = np.array([[255, 128, 0], [255, 153, 51], [153, 204, 255],
                        [230, 230, 0], [51, 255, 51], [153, 255, 153]])

    skeleton = [[1, 2], [1, 3], [2, 4], [3, 5], [2, 3]]

    pose_limb_color = palette[[2, 2, 2, 2, 2]]
    if kpt_color == 0:
        pose_kpt_color = palette[[4, 4, 4, 4, 4]]
    else:
        pose_kpt_color = palette[[1, 1, 1, 1, 1]]
...
def plot_images(images, targets, paths=None, fname='images.jpg', names=None, max_size=640, max_subplots=1, kpt_label=True, steps=2, orig_shape=None, color=None, kpt_color=None):
            ...
            for j, box in enumerate(boxes.T):
                ...
                if labels or conf[j] > 0.1:  # 0.25 conf thresh
                    ...
                    if kpt_label:
                        ...
                        plot_one_box(box, mosaic, label=label, color=color, line_thickness=tl, kpt_label=kpt_label, kpts=kpts[:,j], steps=steps, orig_shape=orig_shape, kpt_color=kpt_color)
                    else:
    ...
    if fname:
        ...
        Image.fromarray(mosaic.astype(np.uint8)).save(fname)  # PIL save
    return mosaic
