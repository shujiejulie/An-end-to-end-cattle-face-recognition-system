...
class MobileFaceNet(Module):
    def __init__(self, fp16=False, num_features=512, blocks=(1, 4, 6, 2), scale=2):
        ...
        self.conv_sep_1 = ConvBlock(128 * self.scale, 512, kernel=(1, 1), stride=(1, 1), padding=(0, 0))
        # self.conv_sep_2 = ConvBlock(512, 512, kernel=(1, 1), stride=(1, 1), padding=(0, 0))
        # self.conv_sep_3 = ConvBlock(512, 512, kernel=(1, 1), stride=(1, 1), padding=(0, 0))
    ...
    def forward(self, x):
        with torch.cuda.amp.autocast(self.fp16):
            for func in self.layers:
                x = func(x)
        x = self.conv_sep_1(x.float() if self.fp16 else x)
        # x = self.conv_sep_2(x)
        # x = self.conv_sep_3(x)
        x = self.features(x)
        return x
